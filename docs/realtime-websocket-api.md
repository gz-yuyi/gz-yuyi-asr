...
草稿文本可能被后续草稿覆盖。客户端应以 `draft_id + revision` 保留最新版本。服务端会按滚动窗口生成多个 `draft_id`，并在最终文本覆盖到对应时间段后停止继续发送旧窗口的草稿更新；因此 `start_ms` 可能不是 `0`。
...
收到 `FinalTranscript` 后，客户端应优先展示最终文本，并隐藏或弱化同一时间范围内已被覆盖的 speaker turn。草稿文本只建议展示尚未被最终文本覆盖的最新窗口，用于表达当前仍在流动的识别结果，不建议和最终文本做两个割裂区域。
