...
- 新注册声纹的 `AudioUrl` 返回可由浏览器稳定播放的音频，并在注册样本有效期间保持可用，不再依赖调用方输入来源的后续可用性。
- 客户端文档收口为外部可观察的请求、响应和协议语义；授权运行机制、处理流水线、模型算法、存储、环境变量及清理策略等服务端实现说明移至 `yuyi-asr` 仓库。

### 修复

- 测试控制台在注册音频不存在、链接失效或浏览器无法解码时显示明确错误，不再只保留不可操作的 `0:00` 播放器。

## [v1.5.1] - 2026-08-01

### 修复

- 测试控制台的动态 GET 请求统一使用 `cache: no-store`，避免浏览器或中间缓存导致外部接口新增声纹后，声纹列表和人员详情仍显示旧数据。
