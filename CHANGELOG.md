...
- 测试控制台离线任务新增说话人分离高级参数，可设置重叠说话修正、已知说话人数、聚类算法、聚类阈值、说话人数范围和 ASR 切段模式。
- 测试控制台离线任务结果下载新增 RTTM 格式入口。
...
- 测试控制台实时转写结果时间线在持续收到转写事件时保持贴底滚动，避免长音频测试时看不到最新片段。
- 测试控制台实时转写草稿展示适配滚动 `DraftTranscript` 窗口，只对同一草稿窗口裁剪已精修前缀，避免后续草稿被旧窗口基线误判后消失。
